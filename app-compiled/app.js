...
angular.module('edhubJobsApp', ['firebase', 'angular-md5', 'ngRoute', 'ngMaterial', 'ngMdIcons', 'smoothScroll', 'ngAnimate']).config(['$routeProvider', '$locationProvider', function ($routeProvider) {
  $routeProvider //-- Google Maps app
  .when('/', {
  ...
  .when('/ycombinator/positions', {
    ...
    resolve: {
      ...
      requireNoAuthRsv: function requireNoAuthRsv(ycAuthSer, $location) {
        return ycAuthSer.auth.$requireSignIn().then(function (authUser) {
    ...
    resolve: {
      // no authenticated user should go to login/signup view
      requireNoAuthRslv: function requireNoAuthRslv(ycAuthSer, $location) {
    ...
    resolve: {
      authRsv: function authRsv($location, ycUsersSer, ycAuthSer) {
      ...
      profileRsv: function profileRsv(ycUsersSer, ycAuthSer) {
    ...
    resolve: {
      channelsRsv: function channelsRsv(ycChannelsSer) {
        return ycChannelsSer.channels.$loaded()["catch"](function (error) {
          console.log('__>> ERROR - There was an error fetching the channels, error: ' + error);
        });
      },
      profileRsv: function profileRsv($location, ycAuthSer, ycUsersSer) {
        return ycAuthSer.auth.$requireSignIn().then(function (authUser) {
    ...
    resolve: {
      messagesRsv: function messagesRsv($route, ycMessagesSer) {
        return ycMessagesSer.forChannel($route.current.params.channelId).$loaded();
      },
      channelNameRsv: function channelNameRsv($route, ycChannelsSer) {
      ...
      profileRsv: function profileRsv($location, ycAuthSer, ycUsersSer) {
        return ycAuthSer.auth.$requireSignIn( // on success callback
        function (authUser) {
          return ycUsersSer.getProfile(authUser.uid).$loaded().then(function (profile) {
  ...
  .when('/landing', {
    ...
    controllerAs: 'signup',
    resolve: {
      unauthApplyRslv: function unauthApplyRslv($route) {
        // sta = Signup To Apply
    ...
    resolve: {
      orgJobAppsRslv: function orgJobAppsRslv($route, edhubJobPostService) {
