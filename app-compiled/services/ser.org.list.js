...
(function () {
  ...
  angular.module('edhubJobsApp').factory('OrgListSer', ['$rootScope', '$firebaseArray', OrgListSerClass]);

  function OrgListSerClass($rootScope, $firebaseArray) {
    var orgListingsRef = firebase.database().ref('orgListings');
    var orgFeedRef = firebase.database().ref('orgFeed');
    var ycOrgFeedRef = firebase.database().ref('ycOrgFeed');
    var orgApplicantsRef = firebase.database().ref('orgApplicants');
    /**
     *
     * @param orgInfo
     * @param orgId
    ...
    function ycUpdateJobPost(jobInfo) {} // D - delete
    ...
    function ycDeleteJobFromOrganization(jobInfo) {
      ...
      timestamp: 0
      */
      delete jobInfo.$$hashKey; //delete jobInfo.$id;

      delete jobInfo.curOrganization; //delete jobInfo.$priority;
