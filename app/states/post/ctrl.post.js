...
(function () {
  ...
  angular.module('edhubJobsApp').controller('PostCtrl', ['$rootScope', 'edhubJobPostService', '$location', 'edhubAuthService', 'OrgListSer', PostClass]);
  ...
  function PostClass($rootScope, edhubJobPostService, $location, edhubAuthService, eOrgListFact) {
    ...
    vm.edhubAuthUser = edhubAuthService.getAuthUser(); //-- Data Model:
    ...
    $rootScope.$on("edhub-list-unauth-org-signup", function (e, data) {
      var jProVal = ""; // delete vm.organization.password; // this works but causes a weird bug in my code :\
    ...
    function listOrgUnauth() {
      edhubAuthService.signup(vm.organization, {
        listOrg: true,
        path: ''
      });
      vm.edhubAuthUser = edhubAuthService.getAuthUser();
