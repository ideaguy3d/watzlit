...
(function () {
    ...
    } // END OF: SerUsersClass()

    // 7 lines
    function SerChannelsClass($firebaseArray) {
        var ref = firebase.database().ref('/channels');
        var arChannels = $firebaseArray(ref);

        // maybe do some custom data transforms on the arChannels
    ...
    function CtrlAuthClass(ycAuthSer, $location) {
        ...
        authCtrl.login = function () {
            ycAuthSer.auth.$signInWithEmailAndPassword(authCtrl.user.email, authCtrl.user.password).then(function (authRes) {
        ...
        authCtrl.register = function () {
            ...
            ycAuthSer.auth.$createUserWithEmailAndPassword(authCtrl.user.email, authCtrl.user.password).then(function (userRes) {
                $location.url('/');
    ...
    function CtrlProfileClass(
        ...
        profileCtrl.updateProfileFeedback = ''; // this simply returns the username of the profile
    ...
    function CtrlChannelsClass(
        ...
        channelsCtrl.profile = profileRsv; // add online presence
        ...
        channelsCtrl.getMessagesFor = function (entityId, messagesFor) {
            ...
            if (messagesFor === 'forChannel') {
                ...
                channelsCtrl.channelName = ycUsersSer.getDisplayName(entityId); // the parameter order I'm passing in could be wrong ???

                ycMessagesSer.forUsers(entityId, channelsCtrl.profile.$id).$loaded().then(function (messages) {
                    channelsCtrl.messages = messages;
                });
            }
        };
    ...
    function CtrlMessagesClass(
        messagesRsv, channelNameRsv, profileRsv
    ) {
        var messagesCtrl = this;
        var profile = profileRsv;
        messagesCtrl.messages = messagesRsv;
        messagesCtrl.channelName = channelNameRsv;
        messagesCtrl.message = '';

        messagesCtrl.sendMessage = function () {
            var message = messagesCtrl.message;
            var messageData = {
                uid: profile.uid,
                body: message,
                timestamp: firebase.database.ServerValue.TIMESTAMP
    ...
    angular.module('edhubJobsApp').factory('ycAuthSer', ['$firebaseAuth', SerAuthClass]).factory('ycUsersSer', ['$firebaseArray', '$firebaseObject', SerUsersClass]).factory('ycChannelsSer', ['$firebaseArray', SerChannelsClass]).factory('ycMessagesSer', ['$firebaseArray', SerMessagesClass]) // CONTROLLERS
